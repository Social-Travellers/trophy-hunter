...
class TrophiesMapViewController: MenuItemContentViewController {
                ...
                if error == nil {
                    ...
                    for eventObj in eventsAroundMe! {
                        ...
                        if let location = event.location {
                            ...
                            debugPrint("Pin added for event")
                        } else {
                            debugPrint("Could not add pin for event")
                        }
                    }
                    self.allEvents = retrievedEvents
                    debugPrint("Events around me: \(eventsAroundMe?.count ?? 0)")
    ...
    func trophyUnlocked() {
        ...
        debugPrint("Trophy has been unlocked")
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        if let mapAnnotation = annotation as? MapAnnotation {
            ...
            if annotationView == nil {
                ...
                let button   = UIButton(type: UIButtonType.infoDark) as UIButton
                button.tintColor = Constants.Color.THEMECOLOR
                button.setImage(image, for: .normal)

                annotationView?.tintColor = Constants.Color.THEMECOLOR
        ...
        if let tappedAnnotation = view.annotation as? MapAnnotation {
            ...
            if let viewController = storyboard.instantiateViewController(withIdentifier: "CameraViewController") as? CameraViewController {
                if let event = self.selectedEvent {
                    ...
                    debugPrint("Location: \(String(describing: event.location))")
                    debugPrint("Description: \(String(describing: event.trophy?.itemDescription))")
